...
export class ConsultaDocumento implements OnInit {
  ...
  get maskDocumento(): string {
    return this.tipoDocumento === 'cpf' ? '999.999.999-99' : '99.999.999/9999-99';
  }

  get placeholderDocumento(): string {
    return this.tipoDocumento === 'cpf' ? 'Ex.: 000.000.000-00' : 'Ex.: 00.000.000/0000-00';
  }
   ...
   * ------------------------------------------------------------------ */
  breadcrumb = {
    items: [{ label: 'Home', link: '/' }, { label: 'Consulta de Documentos' }],
  };
  ...
  public getViewFields(): any[] {
    return [
      ...
      {
        property: 'nome',
        label: this.tipoDocumento === 'cpf' ? 'Nome' : 'Razão Social',
        gridColumns: 9,
      },
      {
        property: 'situacao',
        ...
        gridColumns: 3,
        tag: true,
        color: (v: string) => this.corSituacao(v),
      },
